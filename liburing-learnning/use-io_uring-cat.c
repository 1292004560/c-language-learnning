...
/*
 * read_barrier()：这个宏插入了一个读内存屏障，它确保在该屏障之前的读操作不会被重排序到该屏障之后，从而保证了内存读取的顺序性和可见性。
 * write_barrier()：这个宏插入了一个写内存屏障，它确保在该屏障之后的写操作不会被重排序到该屏障之前，从而保证了内存写入的顺序性和可见性。
 */
...
int io_uring_setup(unsigned entires, struct io_uring_params *p) {
  return (int)syscall(__NR_io_uring_setup, entires, p);
}
...
int app_setup_uring(struct submitter *s) {
   ...
   * 可以在内核中通过 mmap() 调用映射。
   * 虽然完成队列是直接映射进去的, 但提交队列里面有个数组，我们也把它映射进去
   * */
   ...
   * 如果 IORING_FEAT_SINGLE_MMAP 已设置，我们可以不用第二个 mmap() 去映射。
   * */
