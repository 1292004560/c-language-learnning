...
sds sdsnewlen(const void *init, size_t initlen) {
  ...
  s = (char *)sh + hdrlen; /*找到 char buf[]的首地址  通过sh + hdrlen 可以找到
                              buf的地址  sh + len + alloc + flags*/
...
void sdsIncrLen(sds s, ssize_t incr) {
  ...
  case SDS_TYPE_5: {
    ...
    /* 计算出 SDS 的首地址前一个字节的地址。这个字节用来存储 SDS
     * 类型和长度信息。*/
     ...
     * 增加字符串长度 incr 后，新的长度不能超过 32。这是因为 SDS_TYPE_5
     * 类型的字符串长度是用 5 个比特位来表示， 最大值是 31（2^5 -
     * 1），所以不能超过 32*/
...
sds sdstrim(sds s, const char *cset) {
  ...
  len = (ep - sp) + 1;
  /*
   * void *memmove(void *str1, const void *str2, size_t n) 从 str2 复制 n
   * 个字符到 str1， 但是在重叠内存块这方面，memmove() 是比 memcpy()
